...
window.getElapsedSeconds = function() {
    ...
    const actualElapsed = (Date.now() - window.timerState.startTime) / 1000;
    return Math.floor(actualElapsed * window.timerState.speedMultiplier);
};
...
window.updateTimerDisplay = function(timerTimeElement) {
    ...
    // Add minus sign for negative time in flow state
    const sign = window.timerState.currentSeconds < 0 ? '-' : '';
    timerTimeElement.textContent = `${sign}${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
};
...
window.startTimer = function() {
    ...
    if (startButton) {
        ...
        // Change start button text to "Finish" when timer is running
...
window.enterFlowState = function() {
    ...
    // Get UI elements directly instead of expecting them as parameters
    ...
    if (timerStatusElement) timerStatusElement.textContent = 'In flow state - continuing focus';
...
window.pauseTimer = function() {
    ...
    if (startButton) {
        ...
        // Change button text to "Continue" when paused
...
window.updateTimer = function() {
    ...
    // Update timer display - FIXED: removed Math.max() to allow negative values for flow state
    window.timerState.currentSeconds = window.timerState.isBreak 
        ? (window.timerState.isLongBreak ? window.timerState.longBreakTime : window.timerState.breakTime) - elapsedSeconds
        : window.timerState.isFlowState ? -(elapsedSeconds - window.timerState.focusTime) : window.timerState.focusTime - elapsedSeconds;
    ...
    // Update status text for flow state
    if (window.timerState.isFlowState && timerStatusElement) {
        timerStatusElement.textContent = 'In flow state - continuing focus';
    }
...
window.toggleTestMode = function() {
    ...
    if (testModeContainer.style.display === 'none') {
        ...
        window.timerState.speedMultiplier = 1;
    } else {
...
window.setSpeedMultiplier = function(multiplier) {
    window.timerState.speedMultiplier = multiplier;
...
window.setCustomSpeed = function() {
    ...
    if (customInput && customInput.value) {
        const multiplier = parseInt(customInput.value);
...
window.undoLastAction = function() {
    console.log('Undo last action called');
    
    // If there's no last action, we can't undo
    if (!window.timerState.lastAction) {
    ...
    // Restore timer state
    const lastAction = window.timerState.lastAction;
    
    // If we completed a session, remove it
    if (lastAction.type === 'skipToWork' || lastAction.type === 'skipToBreak') {
        if (window.sessionData.sessions.length > 0) {
            window.sessionData.sessions.pop();
    ...
    if (lastAction.sessionData) {
        window.sessionData.currentSession = {...lastAction.sessionData};
    }
    
    // Restore state
    window.timerState.isBreak = lastAction.isBreak;
    window.timerState.isRunning = lastAction.isRunning;
    window.timerState.isPaused = lastAction.isPaused;
    window.timerState.isFlowState = lastAction.isFlowState;
    window.timerState.currentSeconds = lastAction.currentSeconds;
    window.timerState.elapsedBeforePause = lastAction.elapsedBeforePause;
    window.timerState.startTime = lastAction.startTime;
    ...
    // Clear last action and disable undo button
    window.timerState.lastAction = null;
    if (undoButton) {
...
>>>>>>> a3770046ce6fb38291f9704607873be9bdbeb3d9
}; 